...
send_telegram_alert_on_exit() {
  ...
  if [ -x "${PYTHON_BIN}" ] && [ -f "${REPO_ROOT}/scripts/telegram_alert.py" ]; then
    ALERT_JOB="isic2018_train(conn=${CONN_NUM:-unknown},label=${LABEL_MODE:-unknown},epochs=${EPOCHS:-${EPOCHS_OVERRIDE:-unset}})"
...
print_help() {
  cat <<EOF
Usage: sh scripts/isic2018_train.sh [options] [extra train.py args]

Unified ISIC2018 training launcher.
...
Fold selection:
  ...
  example: sh scripts/isic2018_train.sh --folds 1
...
Examples:
  ...
  bash scripts/isic2018_train.sh --conn_num 24
  bash scripts/isic2018_train.sh --label_mode dist_signed --dist_aux_loss gjml_sf_l1 --dist_sf_l1_gamma 1.0
  bash scripts/isic2018_train.sh --label_mode dist_inverted --epochs 30 --folds 1
EOF
}
...
if [ ! -d "${REPO_ROOT}/data/ISIC2018/image" ] || [ ! -d "${REPO_ROOT}/data/ISIC2018/label" ]; then
  ...
  echo "Run scripts/prepare_isic2018_npy.py once before training." >&2
  exit 2
fi
